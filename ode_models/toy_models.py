...
def f_XY(var, t, params):
    ...
    return [
            np.cos(theta1) + np.cos(theta2), # x

            np.sin(theta1) + np.sin(theta2), # y
...
def plot_traj(res, params):
    ...
    ax.scatter(res.y[0], res.y[1], alpha=.8, marker='.')
...
def plot_traj_ax(res, params, ax):
    ax.plot(res.y[0], res.y[1], alpha=.2)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_title('Trajectory')
    #return ax
