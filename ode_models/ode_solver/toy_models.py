...
def f_2D(var, t, params):
    '''Computes the derivative of x at t.'''
    # x, y, theta1, theta2, J1, J2 = var
    theta1, theta2, J1, J2 = var
    # the upper target is defined by (xt, yt)
    # then the lower target is defined by same x but opposite y
    alpha, beta, Ja, Jmax, theta0, omega1, omega2 = params

    return [
            -Ja*np.sin(theta1-theta2) - J1*np.sin(theta1-omega1),   # Theta 1

            Ja*np.sin(theta1-theta2)- J2*np.sin(theta2-omega2),     # Theta 2

            alpha*(Jmax-J1)*theta_heav(theta0-abs(theta1-omega1)) - beta*J1*theta_heav(abs(theta1-omega1) - theta0), # J1
            
            alpha*(Jmax-J2)*theta_heav(theta0-abs(theta2-omega2)) - beta*J2*theta_heav(abs(theta2-omega2) - theta0) # J2
            ]

def f_2D_nofeed(var, t, params):
    '''Computes the derivative of x at t.'''
    # x, y, theta1, theta2, J1, J2 = var
    theta1, theta2 = var
    # the upper target is defined by (xt, yt)
    # then the lower target is defined by same x but opposite y
    Ja, Jmax, theta0, omega1, omega2 = params

    return [-Ja*np.sin(theta1-theta2) - Jmax*theta_heav(theta0-abs(theta1-omega1))*np.sin(theta1-omega1),   # Theta 1

            Ja*np.sin(theta1-theta2)- Jmax*theta_heav(theta0-abs(theta2-omega2))*np.sin(theta2-omega2)     # Theta 2
            ]
